...
This method is called when the weather module tries to fetch the current weather of your provider. The implementation of this method is required for current weather support. The implementation can make use of the already implemented function `this.fetchData(url, method, data);`, which is returning a promise. After the response is processed, the current weather information (as a [WeatherObject](https://docs.magicmirror.builders/module-development/weather-provider.html#weatherobject)) needs to be set with `this.setCurrentWeather(currentWeather);`. It will then automatically refresh the module DOM with the new data.

### `fetchWeatherForecast()`

This method is called when the weather module tries to fetch the weather of your provider. The implementation of this method is required for forecast support. The implementation can make use of the already implemented function `this.fetchData(url, method, data);`, which is returning a promise. After the response is processed, the weather forecast information (as an array of [WeatherObject](https://docs.magicmirror.builders/module-development/weather-provider.html#weatherobject)s) needs to be set with `this.setWeatherForecast(forecast);`. It will then automatically refresh the module DOM with the new data.

### `fetchWeatherHourly()`

This method is called when the weather module tries to fetch the weather of your provider. The implementation of this method is required for hourly support. The implementation can make use of the already implemented function `this.fetchData(url, method, data);`, which is returning a promise. After the response is processed, the hourly weather forecast information (as an array of [WeatherObject](https://docs.magicmirror.builders/module-development/weather-provider.html#weatherobject)s) needs to be set with `this.setWeatherHourly(forecast);`. It will then automatically refresh the module DOM with the new data.
...
[Moment.js](https://momentjs.com/) object of the time/date.

windSpeed

`number`

Speed of the wind in metric: `meter/second`
...
[Moment.js](https://momentjs.com/) object of sunrise.

sunset

`object`

[Moment.js](https://momentjs.com/) object of sunset.

temperature

`number`

Current temperature in metric `celsius degree`.

minTemperature

`number`

Lowest temperature of the day in metric `celsius degree`.

maxTemperature

`number`

Highest temperature of the day in metric `celsius degree`.

weatherType

`string`

Icon name of the weather type.  
...
[  
](https://github.com/MagicMirrorOrg/MagicMirror-Documentation/blob/master/module-development/weather-provider.md)
